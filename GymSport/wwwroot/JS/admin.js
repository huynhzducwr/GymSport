...
// Assume you have a function to fetch product categories

//hàm gọi api lấy danh sách danh mục sản phẩm
...
//Ham goi api lay danh sach san pham
async function fetchProducts(isActive = null) {
    let url = '/api/Product/All';
    if (isActive !== null) {
        url += `?isActive=${isActive}`;
...
//Tạo hàm fetchUserRoles để lấy danh sách role và điền vào dropdown
...
function renderProducts(products) {
    ...
    products.forEach(product => {
        const row = `
            <tr>
                ...
                <td>
                    <select class="status-select" data-productID="${product.productID}">
                        <option value="true" ${product.isActive ? 'selected' : ''}>Kích hoạt</option>
                        <option value="false" ${!product.isActive ? 'selected' : ''}>Không kích hoạt</option>
                    </select>
                    <button class="update-product-status-btn" data-productID="${product.productID}">
                        Cập nhật Trạng thái
                    </button>
    ...
    document.querySelectorAll('.update-product-status-btn').forEach(button => {
        button.addEventListener('click', (e) => {
            const productID = e.target.getAttribute('data-productID');
            const statusSelect = document.querySelector(`.status-select[data-productID="${productID}"]`);
            const isActive = statusSelect.value == 'true';
            console.log('Updating ProductID:', productID, 'to isActive', isActive);
    ...
    item.addEventListener('click', () => {
        ...
        if (page === 'danhmuc-sanpham') {
            ...
            setTimeout(() => {
                ...
                const unactiveProductBtn = document.getElementById('unactive-product-btn');
                const productCategoryTableBody = document.getElementById('product-category-table-body');
                ...
                // Sự kiện cho nút hiển thị sản phẩm Active
                if (activeProductBtn) {
                    activeProductBtn.addEventListener('click', () => {
                        const rows = productTableBody.querySelectorAll('tr');
                        rows.forEach(row => {
                            const isActive = row.querySelector('td:nth-child(4)').textContent.toLowerCase(); // Lấy giá trị trạng thái
                            row.style.display = (isActive === 'yes' || isActive === 'active') ? '' : 'none'; // Kiểm tra trạng thái
                        });
                    });
                }

                // Sự kiện cho nút hiển thị sản phẩm Inactive
                if (inactiveProductBtn) {
                    inactiveProductBtn.addEventListener('click', () => {
                        const rows = productTableBody.querySelectorAll('tr');
                        rows.forEach(row => {
                            const isActive = row.querySelector('td:nth-child(4)').textContent.toLowerCase(); // Lấy giá trị trạng thái
                            row.style.display = (isActive === 'no' || isActive === 'inactive') ? '' : 'none'; // Kiểm tra trạng thái
                        });
                    });
                }

                // Thêm sự kiện cho nút Tạo Danh Mục Sản Phẩm
                document.getElementById('create-product-category-btn').addEventListener('click', () => {
                    const form = document.getElementById('create-product-category-form');
                    form.style.display = form.style.display === 'none' ? 'block' : 'none'; // Ẩn/hiện form
                });
